...
const StateDropDown = ({
  ...
  const getStates = () => {
    ...
    fetch(BASE_URL + `/states/findByCountry/${parentsData?.['ba-country']?.id}`, {
      method: 'GET',
      headers: myHeaders,
    })
      .then(response => {
        switch (response.status) {
          ...
          case 401:
            ...
            throw 'TOKEN EXPIRED';
          case 500:
            Alert.alert('Internal states Server Error');
  ...
  // useEffect(
    ...
    //     if (parentsData?.[dep]) {
    //       getStates(parentsData[dep]);
    //     }
    //   });
    // },
    // (dependencies || [])?.map(dep => parentsData?.[dep]),
  // );

  useUpdateEffect(()=>{
    getStates()
  },[parentsData?.['ba-country']?.id])
    ...
    <View
      ...
      <TouchableOpacity
        ...
        <Text style={dropDownStyles.categoryStyle}>
          {updatedValue?.name || dropDownState?.id ||'State'}
        </Text>
        <Polygon />
        {isOpen ? (
          <Animated.View
